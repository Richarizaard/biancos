...
const ToppingsTag = ({
  ...
  const handleAddTag = async (topping: Topping) => {
    const combinedToppings = [...toppings, topping]
  ...
  return (
    <div className="w-full flex flex-wrap justify-start py-2">
      {toppings.map((topping) => {
        return (
          <div
            className="border-2 px-1 m-1 border-bianco-salmon text-bianco-red rounded-lg"
            key={topping.id}
          >
            {topping.name}
          </div>
